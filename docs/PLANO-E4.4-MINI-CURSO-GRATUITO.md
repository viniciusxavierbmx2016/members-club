...
| **2** 🟨 | ✅ **A ROTA SUBIU (merge `03d00a7`, 10/set/26) — a TELA e o CAPTCHA NÃO.** `POST /api/w/<slug>/register` cria conta + credencial + a **marca**, com `role:"STUDENT"` literal, schema fechado e três provas de que não cria staff (artefato, corpo malicioso e contagem). ⭐ E o **resgate mudou de lugar**: acontece na **VITRINE**, com popup — porque o gate humano achou um **laço fechado** (o card prometia "Resgatar acesso", a página do curso é a **Porta 3**, que a marca não abre, e o 404 devolvia a pessoa à vitrine). ⛔ A Porta 3 **não** foi tocada. **Sobe INERTE: 0 cursos gratuitos em produção.** 🔴 **Falta o item 9.287**: a tela (exige `proxy.ts:66`) e o captcha (`turnstile` = **0** no `src/`). Texto original da etapa: **Cadastro público** (rota + tela): recusa e-mail existente (D5), WhatsApp obrigatório (D7), sem verificação (D6). **NÃO reusar `/api/auth/register`** (DIV-4). ⚠️ **HERDOU 3 REQUISITOS OBRIGATÓRIOS da linha 4** — ver §8; sem eles o funil não fecha | Além das provas de API: os **3 pré-requisitos do §8** atendidos, e **9.139 lido antes de desenhar** |

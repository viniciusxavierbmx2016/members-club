...
| **E4.4** 🟠 | ✅ **ETAPA 2 — A ROTA DE CADASTRO PÚBLICO SUBIU (merge `03d00a7`, 10/set/26), e o RESGATE passou para a VITRINE.** `POST /api/w/<slug>/register` cria conta + credencial + a marca (que até então tinha **0 escritores**); a vitrine e o resgate abriram sozinhos, sem tocar em nenhuma rota deles. ⭐ **O gate humano achou um LAÇO FECHADO** e é a lição do dia: o card prometia "Resgatar acesso", mas levava à página do curso — a **Porta 3**, que a marca não abre por decisão medida do §12.3 — e o 404 devolvia a pessoa à vitrine em silêncio. **O teste por API tinha passado**; o percurso de cliques estava quebrado. ⛔ A Porta 3 segue intocada (byte-idêntica no merge). **Sobe INERTE**: 0 cursos gratuitos, 0 marcas, 0 resgates em produção; 0 de 73 cards da loja mudam. ✅ **ETAPA 2 FATIAS 2c+2d EM PRODUÇÃO (merge `0bc7d82`, 11/set/26) — O FUNIL É ALCANÇÁVEL.** Subiu a **TELA** `/w/<slug>/register` e o **link "Criar conta"** no rodapé da tela de login dos **45 workspaces**. Medido: `register` foi de **307 → 200**, e as **3 telas irmãs** mais `/sw.js`, `/_next/image` e 3 rotas de API **não mudaram de status**. O `proxy.ts` recebeu **1 linha de código** — provado por byte-identidade: o arquivo sem comentários é igual à main com `"|register"` inserido uma vez (**delta = 9 bytes**). A rota de cadastro, o claim, a **Porta 3** e o `workspace-access.ts` conferidos **byte-idênticos por shasum**, com controle positivo provando a sonda viva. **Segue INERTE**: 0 cursos gratuitos, 0 marcas, 0 resgates. ⭐ **Achado do 9.288 (fechado sem fix):** a cor do link parecia fixa em `#818cf8` e **herda** — provado por discriminação em 3 workspaces reais de produção, sem escrever nada (`#ff6b6b`, `#3b82f6`, `#818cf8`, cada um casando com o seu banco). O lilás é o `@default` do schema (`prisma/schema.prisma:92`) e **31 de 45 (69%) nunca customizaram**. ✅ **FATIA 3 EM PRODUÇÃO (merge `83f7dd5`, 11/set/26) — O FUNIL ESTÁ COMPLETO.** O **CAPTCHA** (Cloudflare Turnstile, widget v2, modo *managed*) entrou na tela de cadastro, e **só nela**. ⛔ **FAIL-CLOSED por ordem do dono, invertendo a decisão de 30/08** (§10 do PLANO-E4.4 e item 9.171 registravam fail-open): token ausente, vazio ou inválido ⇒ **403**. Custo escrito: se a Cloudflare cair, ninguém se cadastra. **Provado em produção nos DOIS hosts:** widget no bundle com a sitekey inlinada · CSP `script-src=1 frame-src=1 connect-src=0` · **4 de 4 tentativas sem token → 403 com delta ZERO** em `User`/`WorkspaceCredential`/`WorkspaceMembership` · `/sw.js` byte-idêntico · 3 telas irmãs sem widget (com controle positivo) · vigia 0/0/0 · interruptor 25. ⭐ **Fecha também o 9.171:** o motivo da recusa virou **discriminado** (`not-configured` · `missing-token` · `invalid-token` · `unreachable`), e a frase ao visitante distingue "lado de cá" de "lado do visitante" — foi por ela que se provou, de fora, que a secret está configurada e é aceita pela Cloudflare. ⚠️ **O que a subida NÃO provou:** que um cadastro **legítimo** passa em produção — exige token real de navegador. 🔴 **Abertos desta fatia:** **9.289** (`invalid-input-secret` classificado como culpa do visitante — a fresta do 9.171) e **9.290** (trocar a secret do widget v2, exposta em conversa). ⭐ **E o achado que atrasou a fatia, e por isso a salvou:** a variável `TURNSTILE_SECRET_KEY` na Vercel continha uma **`sk_live_` do Stripe** desde 30/ago. Investigado em somente-leitura: **nada em produção a lia** (0 arquivos na main; os 3 commits da sonda não são ancestrais dela), então nunca teve efeito — **mas `turnstile.ts:63` a enviaria num POST para a Cloudflare no primeiro cadastro.** Corrigida pelo dono antes deste merge. Itens **9.285**, **9.286**, **9.288**. ✅ **PÓS-FUNIL (merge `6fa8761`, 11/set/26): o e-mail de RECOMPRA passou a dizer que a senha continua a mesma (9.291).** Quem já tem credencial e compra de novo recebia e-mail **mudo** sobre a senha — e em **3 de 46** workspaces, com o rótulo órfão *"Senha:"* vazio (**111 alunos já atingidos**, 268 na próxima compra; os outros 41 ficavam mudos). ⛔ O caminho de **conta nova** é **byte-idêntico** nos 4 templates, provado contra a árvore mesclada, com o controle positivo dos 4 de recompra diferindo. ⚠️ **Não se prova por fora que o e-mail chega**: é código de servidor e `/api/admin/test-email` envia de verdade — a confirmação é a próxima recompra real. Abertos: **9.292** (a prévia do produtor só mostra conta nova), **9.293** (os azuis padrão: login vem de `@default` e alcança só os novos; e-mail/vitrine/membros vêm de fallback no código e alcançam a maioria na hora), **9.294** e **9.295** (a aba "Personalizar tela de cadastro", com 2 modelos e HTML próprio). ✅ **O BOTÃO DO E-MAIL JÁ ESTÁ LIME (merge `f0e3e2d`, 12/set/26 — 9.296):** o azul padrão virou `#EFFF20` e o branco **cravado** virou `contrastingTextColor`, no **mesmo ponto do código** — porque branco sobre lime dá **1,11**. ⭐ **Conserto de carona medido:** dos 7 workspaces com cor de e-mail própria, **5 estavam abaixo de 4,5** (o pior, `#ffca10`, em **1,53**) e todos passaram — **5 melhoram, 2 iguais, 0 pioram**. ✅ **VIRADA LEVA 3b (12/set/26, por banco, sem deploy — 9.299): ligados 25 → 31.** Os 6 workspaces onde **NENHUM aluno muda de tela** (`shop-club`, `kingdomacademy`, `consultoriaindividual`, `obs-pronto-esoe`, `formacao-home-office-fho`, `cva`). ⭐ **É teste de ESCALA:** 3.563 alunos passam a viver sob o mecanismo e **zero vê diferença** — 9 dos 10 cursos têm `memberPrimaryColor` própria, e o `??` do `layout.tsx:162` curto-circuita antes de olhar a virada. O alcance do mecanismo saltou de **59 alunos (0,22% da base)** para **3.622**, sem arriscar um pixel. Provado depois de ligar: os 9 cursos com marca intactos (`updatedAt` de nenhum tocado) e o botão de suporte igual, porque as `--member-*` saem de `:root{}` e a marca do produtor vence antes da virada. ⚠️ Exposto: `cva/cva` (1 de 10 cursos) passa a receber o lime, com **0 matrículas ativas** no instante da escrita — o ponto do 9.257. ✅ **LEVA 3a EM PRODUÇÃO (14/set/26, por banco — 9.303): ligados 31 → 41.** Os 10 pequenos, e **é a primeira leva que mexe no pixel**: **720 alunos** passam a ver a área de membros em lime (15 dos 16 cursos; 1 mantém a cor do produtor). Autorizada pela 3b, que rodou **2d13h com 3.654 alunos** sem sinal adverso — 1 ticket, sobre conteúdo, em curso de marca própria; acesso **subiu**. Remedido antes de ligar: 711 → 721 (vendas normais), sem salto de faixa, sem curso novo. ⚠️ Nenhum dos 10 força modo, então o **modo claro** depende do `--member-ink` — e a 10ª regra dele fechou em `3a2d9ba`. Faltam **6**: `ebenezer-2-0` (1.331), `home-office-lucrativo` (2.115), `infinityia` (2.804), `grupo-sm` (5.144), `mentoria-juncao-milionaria` (11.027 — 48% de tudo) e `monetizy-club` (novo, 13/set). ✅ **E O ÚLTIMO PONTO DO MODO CLARO FORA DO PADRÃO FECHOU (merge `3a2d9ba`, 12/set — 9.300):** `globals.css:361` passou a usar `--member-ink` como as 9 irmãs. ⭐ **Zero pixel hoje** (medido: `.text-primary` tem 0 ocorrências dentro de `.course-customized`, com controle positivo), mas **10 das 12 marcas reais ficariam abaixo de 4,5** se a regra fosse usada — `#ffffff` daria **1,00** e o lime **1,11**. ⇒ **isto destrava a leva 4**, cujos 22.421 alunos podem todos escolher o modo claro. ✅ **O 9.297 FECHOU EM PRODUÇÃO (merge `804e506`, 14/set/26), em duas fatias — e a segunda só existiu porque o gate REPROVOU a primeira.** A fatia 1 alinhou a prévia (importando as constantes, sem copiar valor); o gate mostrou que **a prévia estava certa** e quem divergia era o modelo **PADRÃO**, usado por **36 dos 46** workspaces, que nunca passou pelo `emailPrimaryColor`. A fatia 2 pôs o `ctaButton` em lime com texto calculado: **9 tipos de e-mail da plataforma viram**, e os **6 de cor semântica** (convite, alertas de assinatura, suporte) **mantêm a cor** — o recorte sai de graça do `default`. ⛔ E o texto calculado consertou os outros **de carona**: `#f59e0b` 2,15→9,22 · `#10b981` 2,54→7,80 · `#ef4444` 3,76→5,26. **Nenhuma cor semântica virou lime; nenhum contraste piorou.** Os **5** workspaces com cor própria: **byte-idênticos**. Abertos: **9.301** (tocar no seletor troca o modelo da prévia — decisão de produto) e **9.302** (os 2 e-mails que passam azul explícito: manter ou virar). ✅ **VALIDADO EM PRODUÇÃO DEPOIS DA ROTAÇÃO DA SECRET (14/set/26, somente leitura — 9.290 e 9.304): O CADASTRO NÃO PAROU.** A pergunta era urgente porque o funil é **fail-closed**: secret errada ⇒ ninguém se cadastra. ⭐ **E caiu o "não provado" da fatia 3** — o tráfego real trouxe **1 cadastro às 20:35Z**, e a cadeia fecha em `file:line`: `WorkspaceMembership{origin:"PUBLIC_SIGNUP"}` tem **um único escritor** (`register/route.ts:207`); o **único** `POST` (`:46`) chama `verifyTurnstile` na **`:73`**, *antes até do schema*, e recusa em `:76` (as 3 escritas — `:174`, `:198`, `:206` — vêm todas depois); e `turnstile.ts:93` é o **único** `ok:true` do módulo, preso ao `success` da Cloudflare. ⇒ **aquela linha só pode existir se a Cloudflare aceitou a secret de produção E um token real de navegador: o funil está vivo de ponta a ponta.** A vigia E4.4 saiu de 0/0/0 para **1 curso gratuito · 1 resgate · 6 marcas**. Medido junto: **6 de 6** tentativas sem token → **403** nos dois hosts, **todas** com *"Recarregue a página"* (a frase do lado do visitante) e **nenhuma** com *"alguns instantes"* (a do nosso lado) · `/sw.js` byte-idêntico · **0** contas de teste criadas, ⛔ por ordem expressa — a prova saiu da **recusa**, não do sucesso. 🔴 **Mas a rotação NÃO fechou a exposição:** a secret **anterior** ainda devolve `invalid-input-response` (= *aceita*) em vez de `invalid-input-secret`, e a **sitekey é idêntica** nos três lugares (mesmo widget v2). Ou a rotação **não efetivou** (o **9.169** se repetindo), ou é **janela de graça** — **o discriminador é o TEMPO**: repetir a sonda do §10.4 com a chave antiga horas depois. 🔴 **Abertos:** **9.290** (agora com resultado medido, e não fecha antes da 2ª medição) e **9.304** (a chave **nova** também trafegou em chat ⇒ 3ª rotação feita **fora** do chat, validada **por comportamento**, sem nunca ver o valor). ✅ **FATIA B — 9.246 EM PRODUÇÃO (merge `9f1d080`, 15/set/26), 1 arquivo e 1 LINHA.** A bolinha da alternativa marcada deixou de ser `bg-white` fixo e passou a `bg-[var(--member-button-text,#ffffff)]` — molde do 9.243. ⭐ **A descoberta: atingia os DOIS modos, não só o escuro** — nos cursos que recebem o lime pela VIRADA o `--member-card` nem é emitido, então o fallback `#ffffff` voltava a valer também no claro. Medido em Chrome headless: **1,11 → 17,91 nos dois modos**. Alcance: **5 de 6 cursos com quiz · 551 de 552 alunos**. Controles: curso **sem marca byte-idêntico** (3,68) · o **✓ intacto** e a borda `rgb(16,185,129)` emerald CRU · marca **escura → ponto BRANCO** (17,06). ⛔ O estado `incorrect` nunca piora (prova por exaustão dos 2 valores possíveis). Em produção: a regra no **CSS servido**, `/sw.js` byte-idêntico, 7 rotas sem mudança. ⚠️ Não provado: **um aluno real fazendo o quiz** — o chunk é lazy atrás de login e os hashes da Vercel diferem; a prova pública ficou no CSS. 🔴 **Os outros 3 da família NÃO são conserto de CSS — viram DECISÃO DE DESENHO:** **9.245** tem alcance **ZERO** (0 comentários de ADMIN/EQUIPE em 104) · **9.242** tem **0 manifestações** e o item está **desatualizado** (o texto já segue a marca) · **9.244** colapsa só no escuro e **sobrevivem 3 pistas** (anel, borda emerald crua, e o ✓), e mexer nelas **removeria distinção**.
...
| **E3.30 Anexo de suporte: vínculo real + tipo pelos BYTES (9.130 ✅ · 9.305)** 🟠 | ✅ **9.130 EM PRODUÇÃO (merge `975295c`, 14/set/26).** O funil transformou *"qualquer autenticado"* em *"qualquer pessoa da internet"* (47 workspaces com cadastro aberto; a conta nasce logada). ⛔ **O molde da comunidade NÃO serviu inteiro:** `hasRealPlatformLink` trancaria **105 dos 137 produtores (77%)**, que não têm workspace nem curso, e liberaria **27.430 alunos** sem tela para isso. A régua veio das **rotas irmãs** (`canAccessTicket` + `support/tickets:80`); o **sniff por bytes** foi copiado verbatim. Palco: funil **403** com `/api/auth/me` **200** na mesma sessão · EXE/SVG disfarçados **400** · PNG/JPG/GIF/PDF **200** · produtor **sem workspace 200**. Produção: 401 sem sessão (igual à base), `/sw.js` byte-idêntico, `ALLOWED.has(file.type)` em **0** arquivos do artefato. ⚠️ Não provado: produtor real anexando — só o uso confirma. 🔴 **Aberto: 9.305** — a tabela de assinaturas ficou duplicada; assinatura nova entra nas DUAS ou em nenhuma. | ⬜ pendente — achado da frente de PDF na comunidade; **qualquer autenticado**, MIME e extensão escolhidos pelo cliente | — | — |
...
| **E3.35 ✅ FECHADO — a senha-mestra que vazava e a rota que criava ADMIN (9.144 ✅ · 9.135 ✅)** | ✅ **GRUPO FECHADO em 10/set/26, os DOIS itens em produção.** ⭐ **9.135 FECHADO, merge `1e10cd0`: a rota `/api/auth/register` foi APAGADA** — era pública, órfã e cravava `role:"ADMIN"` em `:57`. Medido em produção com 1 conta descartável autorizada (criada e apagada no mesmo comando): **201, `User` 27.996→27.997, `ADMIN` 1→2, e-mail já confirmado**. ⭐ **Ninguém tinha usado: existia 1 única conta ADMIN, a do dono, de 12/abr** — vulnerabilidade ABERTA, incidente NÃO ocorrido, e **o `AuditLog` não registra cadastro**, então a contagem era a única evidência. **Apagar venceu validar** porque a rota tinha 0 chamadores em todo o repo (controle: `register-producer` = 2) e a página `(auth)/register` é só um aviso sem `<form>`. Prova pós-deploy pelo **origem exposto**: **400 → 404**, com `register-producer` seguindo 400, ADMINs em 2 e `/sw.js` byte-idêntico. ⚠️ **A borda é contornável:** pelo domínio o Cloudflare dá 403 em `/api/auth/*`, mas `applyfy-mvp.vercel.app` **não passa por ele** — reforça o **2.4** (origin lockdown), que segue aberto. ⬜ Restava do texto antigo: ✅ **9.144 FECHADO em produção, merge `ae9fdf2` (10/set/26), gate humano 5/5**: `import/route.ts` deixou de buscar (`:128`) e de entregar (`:255`) a `masterPassword`; passa a usar sempre a credencial individual `mc-XXXXXX`. **Alcance do que já vazou, medido:** 11 de 44 ws com master · 9.331 matrículas ativas · **7.618 pessoas** · 18 contas não-STUDENT · **nenhum registro de quem recebeu** (`Enrollment.origin` 100% `UNKNOWN` em 29.626 linhas; a rota não audita; não há log de e-mail). ⭐ **Decisão do dono, consciente e com o número na mão: NÃO rotacionar as masters e NÃO avisar os produtores — o que vazou fica.** O conserto é para frente. Itens novos que a frente abriu: **9.280** (aluno pré-existente sem senha entregue — a master mascarava), **9.281** (gate de `isStaff` no `buildAccessEmail`), **9.282** (runtime do `origin` nunca subiu). ⬜ **Sobra o 9.135**: **9.144**: o import CSV entrega a `masterPassword` do workspace ao aluno (`import/route.ts:255` → e-mail `:401` → coluna "Senha" do CSV `:431`), contradizendo `students/route.ts:273-274` (*"never leaves the server"*, `isMaster:false`). **MEDIDO: 10 de 39 ws têm master configurada** ⇒ VIVO. ⚠️⚠️ **A exposição CRESCE a cada importação — não é dívida estática**, e é isso que o põe à frente. **9.135**: adendo confirma `role:"ADMIN"` hardcoded (`register/route.ts:57`) e que **não há camada de borda** (`proxy.ts:59-61`, sem `middleware.ts`); **veredito segue EM ABERTO** — medir exposição exige ESCRITA e ordem própria do dono | — | — |
| **E3.36 E-mails: destinatário, template, o LOG que mentia e o ORÁCULO POR TEMPO (9.145 · 9.146 · 9.152 · 9.283 ✅ · 9.309 ✅ · 9.306 · 9.307 · 9.308 · 9.310 · 9.311 · 9.312 · 9.313 ✅ · 9.314 · 9.315 · 9.316 · 9.317 · 9.318 ✅ · 9.319 · 9.320 · 9.321)** 🔴 | ✅ **9.283 EM PRODUÇÃO (merge `317d975`, 14/set/26), 1 arquivo.** ⭐ **O defeito era maior que o item:** `sendEmail` **nunca rejeita** (0 `throw`, 4 `return`, o único `await` dentro do `try`), então o `.catch` da rota era **código morto** — o log dizia *"Email enviado"* e **o único caminho que poderia desmenti-lo não existia**. Provado por sonda em runtime (`.then`=true, `.catch`=false). Agora o desfecho sai do valor resolvido, com motivo de nome próprio (`credencial-ausente` · `brevo-<status>` · `destinatario-invalido` · `promessa-rejeitada`). ⚠️ O **status vem antes do nome** porque o minificador reduz a classe do Brevo a uma letra — a 1ª versão gravou `brevo-n-401`; só apareceu no build de PRODUÇÃO. ⛔ A resposta não mudou: os 3 caminhos com `sha256 c955e57777ec0d73` idêntico, no palco e em produção pelo origin; `/sw.js` byte-idêntico. ⛔ E saiu o e-mail da pessoa do log. ⭐ `after()` (nativo do Next, 1º uso no repo) garante que a linha não se perca no freeze. ⚠️ **Não provado:** a linha de SUCESSO ponta-a-ponta — o palco não tem chave do Brevo, e em produção o instrumento só é alcançado por um e-mail que EXISTE (`if (!user) return` em `:60`, antes de `generateLink:82` e `after:130`), o que dispararia recuperação real. A 1ª linha verdadeira virá do 1º pedido real (~126/dia). 🔴 **Abertos por esta fatia: 9.306** (o `.catch` morto em **14** call-sites, incluindo o irmão do ALUNO que era o "molde") · **9.307** (`email.ts:30/:51` logam o endereço, e o `:51` loga o erro cru, que pode ecoar o action_link) · **9.308** (`automation-engine:139` persiste o e-mail em `details`) · 🔴 **9.309 — a anti-enumeração por TEMPO não existe**: 0,807 s para e-mail que existe contra 0,082 s para inexistente, **10×**, porque o vazamento vem do `generateLink` **awaitado**, não do envio. O comentário `:62-68` promete o que a medição desmente. ✅ **9.309 FECHADO EM PRODUÇÃO (merge `3adfefc`, 15/set/26), 2 arquivos.** ⚠️ **ERRATA: eram 1,5×, não 10×** — os 0,807 s iniciais eram arranque a frio. ⭐ **Mas a separação era TOTAL** (0,124 vs 0,093, folga 30 ms): uma amostra por endereço acertava 100%. **(1) Recuperação:** o `generateLink` desceu para dentro do `after()` do 9.283 — medido por etapa, o `findUnique` custa **87 ms nos DOIS** casos e o `generateLink` **58 ms só para quem existe**; ficou **1,01×**, faixas sobrepostas, e **44% mais rápida** para o legítimo. **(2) Login do aluno:** quem não existe passa a gastar o mesmo trabalho — ⚠️ o **hash falso sozinho resolveria 11%** (scrypt 38 ms de 328 ms de folga); o grosso são as **idas ao banco**, então o alvo falso **repete as consultas**; ficou **0,99×**. ⛔ Não é piso artificial. ⭐ **Garantia estrutural:** o bloco vive dentro de `if (!target)` — **ninguém que hoje entra passa por ali**; matriz de **11 personas: 11/11 igual**. ✅ Em produção a **previsão falseável bateu**: o lado "não existe" do login **subiu 0,166 → 0,245 s**, e o da recuperação (não tocado) ficou. `sha` da recusa **2f3e341da51e1724 idêntico**, 3 caminhos da recuperação idênticos, `/sw.js` byte-idêntico. ⚠️ **Não provado em produção: o lado "existe"** — exigiria disparar contra pessoa real. 🔴 **Abertos: 9.310** (a equalização por réplica envelhece em silêncio) · **9.311** (workspace bloqueado ainda denuncia por conteúdo: 503 × 401) · **9.312** (`producer-login` 1,4× e `w/<slug>/forgot-password` 1,2× seguem vazando, com sobreposição). | ⬜ pendente — nasceu da investigação E4.4 etapa 5. `resend` dispara recovery global sem `isStaff` (a palavra não existe no arquivo) · `producer/students/[id]/enrollments` manda acesso sem senha, sem `isStaff` e **sem garantir credencial** · destinatário CRU × normalizado na mesma requisição | — | — | ✅ **9.313 (FATIA 1 de 5) EM PRODUÇÃO — merge `a9b6d34`, 15/set/26, 3 arquivos.** 🔴 O defeito era de RECEITA: o retorno de `sendCustomAccessEmail` era descartado e o `logWebhook` gravava `SUCCESS` hardcoded; como a dedup lê esse SUCCESS em 60 s, um e-mail falhado fazia o retry do gateway **PULAR** o envio. Medido: **685 janelas de retry em 90 d, 539 de compra aprovada, 357 pessoas** — e ⚠️ **quantas perderam o e-mail é invisível por construção** (controle pareado: 29,1% × 25,6%, razão 1,14×, sem sinal e sem poder). ⭐ Conserto **sem migração**: o desfecho vai em `errorMessage` (a tela já pinta de vermelho em qualquer linha) e em `_emailAcesso` no `rawPayload` (o padrão do `_idempotency`). ⚠️ **O `status` continua SUCCESS de propósito** — reprovado que só a dedup lê o status; mudá-lo seria a fatia 2. ⛔ Nenhum `.catch` removido (2/6/5 idênticos) — `sendCustomAccessEmail` pode rejeitar e tirá-lo quebraria a MATRÍCULA. Gate: dedup, matrícula, catch e 200 **byte-idênticos** nos 3. Palco: webhook real 2× em 7 s → `falhou` + `pulado-duplicata`, **200 nos dois e matrícula ACTIVE criada**. ⚠️ Não provado: o ramo `enviado` (palco sem chave do Brevo). 🔴 **Abertos: 9.314** (a dedup exigir `_emailAcesso=enviado` — muda comportamento) · **9.315** (o `/resend` mente, e o `accessLink` que salvaria tem **0 usos na tela**) · **9.316** (os outros 5 call-sites — ⛔ sem tirar os `.catch`) · **9.317** (não existe reprocessamento). ✅ **9.318 (FATIA 1A) EM PRODUÇÃO — merge `e7dff3c`, 16/set/26, 1 arquivo.** 🔴 O mesmo defeito do 9.283/9.313, agora no CRON DE COBRANÇA: `sendBillingEmail` descartava o retorno de `sendEmail` e só podia falhar por um `catch` que **nunca abre**, então o `BillingReminder` era gravado mesmo com o aviso não enviado — e o tipo entrava no `sentTypes`, **carimbando o aviso como entregue PARA SEMPRE**. ⚠️ A promessa é de `4052d2e` (31/mai/26), um commit que **criou o portão e os três comentários no mesmo diff** e nasceu no-op; a mentira é tão convincente que um agente de leitura desta sessão a repetiu duas vezes. 📏 **Medida que corrige o enunciado:** o aviso é perdido **POR DEGRAU, não para sempre** — 3 dos 6 degraus de aviso são JANELAS que fecham sozinhas (`:44`, `:50`, `:56`) e 3 são ABERTOS (`:73`, `:99`, `:105`); teto de **3 chamadas por assinatura por rodada**, 1 rodada/dia. ⭐ **Decisão do dono: só os 3 pré-vencimento**, porque gatear os abertos criaria **retentativa diária sem teto** — eles são o **9.320** e precisam de limite antes. ⭐ **O bisturi foi o argumento default** (`exigirEnvio = false`): as 3 chamadas de fora saem byte-idênticas e o `&&` curto-circuita no primeiro operando. ✅ Gate reprovado **na árvore mesclada**: suspensão, os 3 pós-vencimento, as 3 travas e os `.catch` todos **byte-idênticos**. ⭐ Palco com **discriminador dos dois lados na mesma rodada** (os 4 bonecos com o MESMO e-mail falhando): os 3 do escopo → `ret=false`, reminder 0, motivo `credencial-ausente`; o controle `after_1d` → `ret=true`, reminder 1, `"Sent after_1d to…"`. ⭐ Prova em produção **pelo artefato do servidor**: 3 chamadas com `,!0)` e 3 sem o 4º argumento, com a assinatura `l=!1` e o corpo `l&&!u?.success`. ⚠️ **Não provado: o cron rodando** — ele roda 1×/dia e **hoje não age** (135 subscriptions, **135 sem `currentPeriodEnd`**, 0 elegíveis, 0 `BillingReminder` na história). Por isso subir agora é o momento **mais seguro e mais urgente** ao mesmo tempo. 🔴 **Abertos por esta fatia: 9.319** (🔴 **não existe TELA de cobrança em lugar nenhum** — 0 ocorrências de `BillingReminder` em `.tsx` com controle positivo de 57/15/4, nenhuma API o devolve, e as linhas COM e SEM carimbo são **indistinguíveis campo a campo**; quando a suspensão cortar os alunos, **ninguém tem onde olhar**) · **9.320** (os 3 pós-vencimento + limite de tentativa) · **9.321** (a suspensão: o portão é no-op e a escolha entre *não suspende* / *suspende e registra* / *suspende após N* é **decisão de produto pendente**).
...
| **E3.42 Furos pré-existentes achados na investigação do vínculo (9.172 · 9.173 · 9.174 · 9.175 · 9.176 · 9.177)** 🔴 | ⬜ pendente — nasceu da investigação read-only da E4.4 etapa 5 (31/08), que perguntava *"ver a vitrine sem matrícula alcança conteúdo?"*. Alcança — e ⭐ **os dois piores NÃO são criados pela mudança: já valem hoje para qualquer conta autenticada**. **9.172 🔴** `courses/by-slug/[slug]/route.ts` (156 linhas) tem `getCurrentUser()` → 401 e **nada mais**: `include` devolve aulas e `videoUrl` cross-tenant, e `api/search:31-42` entrega o slug sem escopo de workspace (oráculo de enumeração, `LIMIT 5`). ⭐ É a **rota IRMÃ** da que o `ee032e5` fechou — aquele tocou só o `/init`. **9.173 🔴** `lessons/[id]/quiz` sem gate de tenant; o POST grava tentativa, dispara `QUIZ_PASSED` e devolve `correctOptionId` (o gabarito). **9.174 🟠** `by-slug/[slug]/init` gateia por WORKSPACE e paga em CURSO — **é o furo que a etapa 5 amplificaria**. **9.175 🟠** cadeado morto no módulo (`locked = hasAccess && !released`). **9.176 🟠** dois criadores de `WorkspaceCredential` sem vínculo — a razão de credencial **não** poder ser pertencimento. **9.177 🟠** `w/[slug]/lives` com `include` sem `select` (URL de transmissão e gravação na listagem) e filtro role-blind. ⛔ **ORDEM OBRIGATÓRIA: 9.172, 9.173 e 9.174 fecham ANTES do cadastro público** — hoje exigem uma conta na plataforma; depois exigiriam só um cadastro grátis. ✅ **9.172 FECHADO e EM PRODUÇÃO (merge `b140e50`, 31/08)** — a rota irmã foi **removida** (fix por REMOÇÃO, +74/−160) e a tela de módulo passou para a gêmea com `?scope=module`; régua nova: matrícula **ATIVA** · dono · ADMIN · colaborador **com aquele curso no escopo**; ⚠️ **matrícula VENCIDA deixa de abrir o curso — mudança deliberada, aprovada**. Gates 9/9 por API + humano 4 blocos. ⚠️ **9.174 fechou SÓ A PODA** (`videoUrl` fora do payload de estrutura, `select` explícito com cobertura 9/9 contra o `information_schema`); **o gate segue por WORKSPACE** porque exigir matrícula ali quebraria o checkout (`init:83-89`) — a metade do gate **continua aberta**. ✅ **9.173 FECHADO e EM PRODUÇÃO (merge `c173a59`, 31/08)** — gate no quiz por reuso de `checkLessonAccess` (mesma família do 9.172: matrícula ATIVA · dono · ADMIN, com short-circuit de staff) e `correctOptionId` agora respeitando `showAnswers`. ⭐ **O short-circuit era obrigatório e isso foi MEDIDO**: 15 tentativas de 3 produtores DONOS, e a quebra seria **muda** (`lesson-quiz.tsx:62`→`:119 return null`). 📏 Furo **não explorado** e **0** automações `QUIZ_PASSED`. Gates: 9 provas por API + humano 4/4. O que ficou de fora virou **9.181** e **9.182** (grupo E3.44). 🟠 **Faltam do grupo: 9.175 · 9.176 · 9.177**, mais os itens novos **9.178** (chunk de painel entregue ao navegador do aluno) e **9.179** (`GET /api/admin/platform-settings` anônimo e que escreve). Laudo no **§11 do PLANO-E4.4**. | `aa7aafd` (registro) · `b140e50` (fix 9.172) | 2026-08-31 |
| **E3.43 VTurb: a dica do editor não lista o quarto provedor (9.180)** 🟢 | ⬜ pendente — nasceu do merge do suporte a VTurb (`2421c01`, 31/08). `lessons-manager.tsx:171` diz *"vídeo (YouTube, Vimeo, Panda Video)"* e a lista **envelheceu no mesmo commit** que aceitou o quarto provedor; o `placeholder` do campo (`:344`) é genérico e não lista nada, então essa dica é o **único** lugar que informa o produtor. ⓘ **Não quebra nada** — quem colar VTurb funciona; é problema de **descoberta**. ⚠️ O comando que abriu o item apontava `:347` (a condição do checkbox `hideYoutubeChrome`); o endereço certo é **`:171`**, conferido antes de registrar. | — | — |
| **E3.44 Quiz: o que o gate do 9.173 NÃO cobriu (9.181 · 9.182)** 🟠 | ⬜ pendente — nasceu do fix do **9.173** (31/08). **9.181**: `checkLessonAccess` resolve tenant e matrícula, mas **não** cobre drip, lock de automação nem `releaseAt` — o player cobre (`view/route.ts:123-156`) ⇒ o quiz de uma aula ainda trancada é respondível, e ele é o gatilho que pode acionar `UNLOCK_MODULE`. **9.182**: `QuizAttempt` não tem `@@unique` e o POST usa `create` ⇒ N envios = N tentativas, N automações, N e-mails/pushes. 📏 Ambos **sem dano hoje**, por medição: média 1,19 tentativa por par pessoa-quiz (máx. 12, de um produtor testando) e **0 automações QUIZ_PASSED** configuradas. ⚠️ O 9.182 precisa de **regra de produto** antes de código. | — | — |
| **E3.46 VIRADA DA IDENTIDADE — o interruptor por workspace e as levas (9.255 ✅ · 9.257 ✅ · 9.259 ✅ · 9.299 ✅ · 9.303 ✅ · 9.322 ✅ · 9.323 · 9.324)** 🔴 | ⚠️ **Esta frente NÃO tinha linha no ROADMAP até 16/set/26** — os itens viviam só no PLANO-MESTRE. Criada aqui para o grupo existir. **O mecanismo (9.255):** `Workspace.memberBrandDefault`; com ele ligado, todo curso SEM `memberPrimaryColor` recebe o lime `#EFFF20` por CÓDIGO (`layout.tsx:162-164`), sem escrever nada no banco do curso. Zero query nova (entra no mesmo `select` do `forceTheme`) e fail-safe no `catch`. **AS LEVAS, por alunos que mudam de tela:** L1 (9.257) 16 ws / **0** · L2 (9.259) 9 ws / **20** · L3b (9.299) 6 ws / **0** · L3a (9.303) 10 ws / **732** · **L4a (9.322) 4 ws / 6.296**. Ligados hoje: **45 de 47**. ⭐ **L4a, 16/set/26:** Monetizy Club (0) · Ebenézer 2.0 (1.354) · Home Office Lucrativo (2.145) · InfinityIA (2.805). O salto é o maior da série — o maior ws já ligado tinha **193** alunos e os 41 somavam 788; a exposição foi de **788 → 7.076 (9,0×)**. Autorizado porque o mecanismo **não tem custo de escala**: o risco é visual e por curso. 5 travas no instante da escrita + `updateMany` com guarda dupla (`id IN` **E** `memberBrandDefault=false`) → 4 linhas. ⭐ **Sinal da L3a com 43,2 h:** 0 tickets · acesso da coorte 3,01%→8,61% (2,86×) contra 0,86%→1,78% do controle (2,06×) · progresso 256→256 com janelas iguais (série 277·327·144·256·256). ⚠️ Minha 1ª leitura usou janelas de tamanhos diferentes e sugeriu queda — corrigido. 🔴 **A L4a ABRIU DOIS DEFEITOS, ambos PRÉ-EXISTENTES nos 41: 9.323** (`.course-customized` liga `opacity: .55–.85` em texto secundário de curso que nunca customizou, porque `--member-text` só sai com `memberTextColor` — `text-gray-500` **4,83 → 2,33** no claro; **56 de 70** cursos ligados; ⭐ a **vitrine já resolveu com gate duplo** e o member tem gate único) **e 9.324** (`rich-text-editor:353/:431/:641` — `#ffffff` sobre lime = **1,11:1**, porque `--producer-button-text` não existe na rota do aluno). ⛔ **SOBRA A LEVA 4b:** Mentoria Junção Milionária (**11.065**) + Grupo SM (**5.140**) = **16.187**. ⭐ **Ela NÃO deve sair antes do 9.323** — levaria a exposição de 7.076 para 23.263. | — | — |
| **E3.45 Achados da investigação da MARCA de pertencimento (9.183 · 9.184 · 9.185 · 9.186 · 9.187 · 9.188)** 🟠 | ⬜ pendente — nasceram da investigação read-only do **formato da marca** (31/08), a base do desenho do cadastro público. **9.183 🟠** o `GET` de tags não escopa por workspace ⇒ o produtor A lê as tags que B/C/D aplicaram (o POST e o DELETE validam; só o GET não). **9.184 🟠** os recortes de live são **cegos a papel** (`if user.role === "STUDENT"` em 4 pontos) ⇒ quem carrega conta PRODUCER escapa de `COURSE_ONLY` e de sala fechada. **9.185 🟠** `/api/courses` devolve o catálogo **cru** (sem `select`): `externalProductId`, `checkoutUrl`, `supportEmail`, `ownerId` — contraste com a vitrine, que tem recorte curado. **9.186 🟢** `HAS_TAG` é **inerte** sem matrícula ATIVA: o produtor tagueia, filtra, monta automação e ela nunca alcança, **em silêncio**. **9.187 🟢** a marca abre as portas mas é **invisível para a navegação** (`/api/student/workspace` devolve 404 sem `Enrollment` ACTIVE) — **a fatia do cadastro terá de tratar**. **9.188 🔵 FUTURO**: bloqueio / lista negra do produtor — hoje se tira o CURSO, não a PESSOA; com a marca, bloquear vira o complemento natural. ⚠️ 9.183/9.184/9.185 são **pré-existentes**; a marca **amplia** a população, não cria o furo. Decisões canônicas no **§12 do PLANO-E4.4**. | — | — |

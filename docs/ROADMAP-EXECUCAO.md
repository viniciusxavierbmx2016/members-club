...
| **E3.41 Turnstile: rotacionar a secret + o gate de config (9.169)** 🟠 | ⬜ pendente — nasceu do ENCERRAMENTO da sonda do Turnstile (31/08). **(a)** a `TURNSTILE_SECRET_KEY` circulou fora do contrato de segredo (nada saiu do git — `git grep` em HEAD → 0); rotacionar custa **ZERO** enquanto o widget não estiver em uso. **(b)** 🔴 o que faz disto mais que rotação: site key e secret **se parecem** (mesmo prefixo, campos vizinhos no painel; 24 × 35 chars) e trocá-las **publica a secret no bundle do navegador** — e **"quase funciona"**, porque a secret valida no servidor. Um gate que pergunte só *"a env existe?"* **não pega**. O discriminador que pega está no §10.4 do PLANO-E4.4. **(c)** com o captcha **fail-open** e a CSP **sem report-uri**, "config errada" e "Cloudflare caiu" são hoje **indistinguíveis** | — | — |
